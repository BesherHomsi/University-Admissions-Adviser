...
def editProfile(id,rout):
	print("""<div class="container">
        <div class="row">
            <div class="col-sm-6 col-sm-offset-3 col-xs-12">
                <form name="editStudentProfile" action=%s method="POST"> 
                    <hr />
                    <div class="row">
					...
					<input type="text" name="studentId" id="studentId" class="form-control" value=%s placeholder="studentId" style="visibility: hidden;">
                    <hr />
                    <div class="row">
...
try:
	...
	if "HTTP_COOKIE" in os.environ:
		...
		if row:
			navBar(row[5])
			if(row[7] == "s"):
				editProfile(row[0],"submitEditProfileStudent.py")	
			if(row[7] == "u"):
				editProfile(row[0],"submitEditProfileUniversityOfficial.py")
			if(row[7] == "a"):
				acadMenu()
