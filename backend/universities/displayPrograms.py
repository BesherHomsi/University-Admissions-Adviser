...
def displayPrograms(name, id, program):
    print("""
	<div class="container">
        <div class="row">
			<h2>%s</h2>
	...
	<div class="container">
        <div class="row">
            <div class="col-sm-6 col-sm-offset-3 col-xs-12">
                <form name="addprogram" action="addApplicationForm.py" method="POST"> 
                    <hr />
                    <div class="row">
                        <div class="col-xs-12 col-sm-6 col-md-6">
                            <div class="form-group">  
                                <select class="form-control" name ="programName" id="programType">"""% name)
    for i in program:
    ...
    print("""</select>
                    ...
                    <div class="row">
                        <div class="col-xs-12"><input type="submit" value="Add Applicaiton" class="btn btn-primary btn-block btn-lg">
                        </div>
                    </div>
                </form>
            </div>
...
try:
    ...
    if "HTTP_COOKIE" in os.environ:
        ...
        if row:
            ...
            uni = c.fetchone()
            c.execute('''SELECT * FROM university_program WHERE UniversityID = %s''' % (q,))
            program = c.fetchall()
            navBar(row[5])
            if program:
                displayPrograms(uni[1], q, program)
